...
from .core import IndoorORCASim, IndoorORCASimConfig
...
class MultiAgentNavigation:
    ...
    """
    def __init__(self, map_processor: MapProcessor, num_agents: int = 1) -> None:
        self.sim = IndoorORCASim(IndoorORCASimConfig())
        self.map_processor = map_processor

        if num_agents < 1:
            raise ValueError('Number of agents must be greater than 1.')
        
        self.num_pedestrians = num_agents
        self.agents = self.sim.add_agents(num_agents)
        ...
        #TODO: Load agents and obstacles to Simulator
